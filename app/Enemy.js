...
export class Enemy extends Node {
    ...
    moveEnemy(dt, time) {
        ...
        this.updateMatrix();
        if (vec3.distance(this.translation, this.wp[this.currWaypoint + 1].translation) <= 0.05 && this.currWaypoint < this.wp.length - 1) {
            this.currWaypoint ++;
        }
    ...
    reachedEnd() {
        if (this.currWaypoint == this.wp.length - 1) 
            return true;
